...
sub get_tag {
   ...
   if ( $self->info->lyrics && not $self->options->{lyricsoverwrite} ) {
      ...
      unless ($lyrics) {
         ...
         $lyrics = $self->get_lyrics_uc( $self->info->artist, $self->info->title, $self->info->album );
      }

      unless ($lyrics) {
         $self->status("Checking HOL Lyrics...");
         $lyrics = $self->get_lyrics_hol( $self->info->artist, $self->info->title, $self->info->album );
      }

      #I abused Lyrics mania and now we are not friends anymore.
      #
      #unless ($lyrics) {
      ...
      unless ($lyrics) {
         ...
         $lyrics = $self->get_lyrics_leos( $self->info->artist, $self->info->title, $self->info->album );
      }
...
sub fetch_url {
   ...
   my $res =
     $self->{rget}->fetch_url( url => $url, file => "/tmp/lyrics/$file", post => $post, use_cache => $cache );
   if ( ref $res ) {
      return $res->content;
   }
   else {
...
sub get_lyrics_leos {
   ...
   my $res =
     $self->fetch_url(
      "http://www.leoslyrics.com/advanced.php?artistmode=1&artist=${artist}&albummode=1&album=&songmode=1&song=${song}&mode=0"
     );

   #print $res->content;

   if ( $res =~ /listlyrics.php.hid=([^"]+)/ ) {
      my $hid = $1;
...
sub get_lyrics_leos_byhid {
   my $self = shift;
   my $hid  = shift;
   my $res  = $self->fetch_url("http://www.leoslyrics.com/listlyrics.php?hid=$hid");
   $res =~ s/[\r\n]//g;
   if ( $res =~ /(\<font face[^>]*\>(.*?)\<\/font>)/m ) {
      my $lyrics = $1;
      $lyrics =~ s/\<font[^>]*\>//g;
      $lyrics =~ s/\<\/font\>//g;
      $lyrics =~ s/\<br[^>]*\>/\n/g;
...
sub get_lyrics_mania_song {
   ...
   if ($ret =~ /The lyrics you requested is not in our archive yet/) {
      return undef;
   }
   if ($ret) {
      return $ret;
...
sub get_lyrics_uc {
   ...
   my $artists = {};
   $self->get_lyrics_uc_alpha(lc(substr($self->simplify($artist),0,1)), $base, $artists);
