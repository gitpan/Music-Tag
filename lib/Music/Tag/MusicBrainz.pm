...
sub get_tag {
   ...
   if  (($self->options->{skip_seen}) && (length($self->info->mb_trackid) == 36)) {
      $self->status("Skipping previously looked up track with mb_trackid " . $self->info->mb_trackid);
   }
   else {
...
sub artist_info {
   ...
   my $maxscore = 0;
   my $artist = undef;
   if (( defined $self->info->mb_artistid ) && ( $self->info->mb_artistid ) && ( not $self->options->{ignore_mbid})) {
	  ...
	  if ($s > $maxscore) {
       ...
       if ($maxscore > $self->options->{min_artist_score}) {
           ...
           $self->status("Artist ", $artist->name, " won election with ", $maxscore, "pts, but that is not good enough");
	   return;
       }
       else {
   ...
   if ( $artist->{name} ) {
      unless (( defined $self->info->artist ) and (( $self->info->artist ) eq ( $artist->{name} )) ) {
...
sub album_info {
   ...
   my $params  = {LIMIT => 200};
   my $release = undef;
   if (( defined $self->info->mb_albumid ) && ( $self->info->mb_albumid ) && (not $self->info->options->{ignore_mbid})) {
      ...
      foreach (@{$releases->releases}) {
         my $s = 0;
         if (( defined $self->info->mb_albumid ) and ( $self->info->mb_albumid eq $_->id ) and (not $self->options->{ignore_mbid})) {
            $s += 256; 
         }
         if ( $_->{title} eq $self->info->album) {
            $s += 64; 
         }
         if ((length($_->{asin}) > 8 ) && ($_->{asin} eq $self->info->asin)) {
            $s += 32; 
         }
         if ( $self->simple_compare( $_->{title} , $self->info->album, .80 )) {
            $s += 32; 
         }
         if (( defined $self->info->totaltracks ) and (( $self->info->totaltracks ) == ( $_->track_list->{count} )) ) {
	    $s += 16;
	 }
	 if ($s > $maxscore) {
	    $release = $_;
      ...
      if ($maxscore > $self->options->{min_album_score}) {
         ...
         $self->status("Highest score of " . $maxscore . " to " .$release->title . " is too low");
	 return;
      }
      else {
...
sub track_info {
   ...
   foreach my $t (@{$tracks}) {
      my $s = 0;
      if ((defined $self->info->mb_trackid) && ($self->info->mb_trackid eq $t->{id}) && (not $self->info->{ignore_mbid})) {
          $s += 128;
      }
   ...
   if ($maxscore > $self->{min_track_score}) {
      ...
      $self->status("Highest score was " . $maxscore . " for " . $track->title . ", but that is not good enough, skipping track info.");
      return;
   }
   else {
   ...
   if (($countrycode) && (not ((defined $self->info->countrycode) && ($self->info->countrycode eq $countrycode)))) { 
     $self->info->countrycode($countrycode);
     $self->tagchange("countrycode");
   }
   if (($releasedate) && (not ((defined $self->info->releasedate) && ($self->info->releasedate eq $releasedate)))) { 
