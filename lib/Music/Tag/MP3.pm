...
=cut
    ...
    $self->info->framesize( $self->{_mp3}->frame_len() );
...
=cut
    ...
    eval {
        $self->info->title( decode_uni( $self->{_mp3}->title ) );
        $self->info->artist( decode_uni( $self->{_mp3}->artist ) );
        $self->info->album( decode_uni( $self->{_mp3}->album ) );
        $self->info->tracknum( decode_uni( $self->{_mp3}->track ) );
        $self->info->comment( decode_uni( $self->{_mp3}->comment ) );
        $self->info->year( decode_uni( $self->{_mp3}->year ) );
        $self->info->genre( decode_uni( $self->{_mp3}->genre ) );
    };
    warn $@ if $@;
...
=cut
    ...
    if ( exists $self->{_mp3}->{ID3v2} ) {
        ...
        my $mbid = $self->{_mp3}->{ID3v2}->get_frame('UFID');
		if (ref $mbid) {
...
=cut
   ...
   $self->{mp3info} = MP3::Info::get_mp3info($filename);
